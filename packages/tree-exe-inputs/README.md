...
`tree-exe-inputs` hosts the concrete input adapters used by the TreeExe runner and orchestrator. Each input encapsulates how external events enter the system (HTTP routes, Bree-backed scheduler, future AMQP/MQTT transports) without coupling the runner core to specific transport details.
