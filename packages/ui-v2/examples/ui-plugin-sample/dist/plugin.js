...
export const SampleSidebarPanel = () => {
    ...
    return React.createElement('div', { className: 'panel stack' }, React.createElement('h3', null, 'Sample Plugin Sidebar'), React.createElement('div', { style: { fontSize: 13, color: 'var(--muted)' } }, `Flows: ${flows?.length ?? 0}`), React.createElement('div', { style: { fontSize: 13, color: 'var(--muted)' } }, `Selected: ${selection?.selectedNodeId ?? 'None'}`));
};
export const SampleInspectorSection = () => {
    ...
    return React.createElement('div', { className: 'panel stack' }, React.createElement('h3', null, 'Sample Inspector'), React.createElement('div', { style: { fontSize: 13, color: 'var(--muted)' } }, `Active flow: ${activeFlow?.name ?? 'Unknown'}`), React.createElement('div', { style: { fontSize: 13, color: 'var(--muted)' } }, `Selected node: ${selection?.selectedNodeId ?? 'None'}`));
};
export const SampleRightRailPanel = () => {
    const React = ensureReact();
    const api = usePluginApi?.();
    const catalog = api?.selectors.useCatalog();
    return React.createElement('div', { className: 'panel stack' }, React.createElement('h3', null, 'Sample Right Rail'), React.createElement('div', { style: { fontSize: 13, color: 'var(--muted)' } }, `Closures: ${catalog?.availableClosures.length ?? 0}`));
};
export const SampleCanvasOverlay = ({ nodes = [] }) => {
    const React = ensureReact();
    const first = nodes[0];
    if (!first)
        return null;
