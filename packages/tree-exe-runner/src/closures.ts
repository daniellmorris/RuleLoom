...
import TreeExeEngine, { resolveDynamicValues, type ClosureDefinition, type ClosureContext } from 'tree-exe-engine';
...
export async function buildClosures(
  ...
  for (const entry of closureConfigs) {
    if (entry.type === 'template') {
      ...
      closures.push(...createCoreClosures());
      ...
      closures.push({
        ...
        handler: async (state, context) => {
          ...
          if (!engine || !(engine instanceof TreeExeEngine)) {
            throw new Error('Flow closures require execution within a TreeExeEngine context.');
          }
