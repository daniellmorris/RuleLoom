...
import RuleLoomEngine, { resolveDynamicValues } from 'rule-loom-engine';
import { importClosureModule } from './config.js';
function buildSetStateClosure(entry) {
...
function buildRespondClosure(entry) {
...
async function buildModuleClosures(entry, baseDir, logger) {
...
export async function buildClosures(closureConfigs, baseDir, logger) {
    const closures = [];
